...
import xmu.oomall.mapper.ShareMapper;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.stereotype.Repository;
import xmu.oomall.util.RedisUtil;
...
public class ShareDao {
    ...
    public ShareRule getShareRule(Integer id)
    {
        if(redisUtil.hasKey(String.valueOf(id))){
            logger.debug("redis has the key");
            String temp=new Gson().toJson(redisUtil.getValue(String.valueOf(id)));
            logger.debug(temp);
            ShareRulePo shareRulePo= new Gson().fromJson(temp,ShareRulePo.class);
            return new ShareRule(shareRulePo);
        }else{
            logger.debug("it will come into mybatis");
            ShareRulePo shareRulePo = shareMapper.getShareRuleByGoods(id);
            logger.debug(shareRulePo.toString());
            ShareRule shareRule=new ShareRule(shareRulePo);
            redisUtil.addKey(String.valueOf(shareRule.getGoodsId()),new Gson().toJson(shareRulePo),48, TimeUnit.HOURS);
            return shareRule;
        }
    }
    ...
    {
        //查看shareRule是否存在
        ShareRulePo shareRulePo=shareMapper.getShareRuleById(id);
        if(shareRulePo!=null){
            redisUtil.del(String.valueOf(shareRulePo.getGoodsId()));
        ...
        if(shareMapper.getShareRuleById(id)!=null)  {
            if(shareRulePo.getGoodsId()!=null){
                ...
                if(shareMapper.alterShareRule(shareRulePo, id) != 0) {
                    redisUtil.del(String.valueOf(shareMapper.getShareRuleById(id).getGoodsId()));
                    return shareRulePo;
                    }
                else {
                    shareRulePo.setId(-3);
