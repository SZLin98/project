...
import xmu.oomall.domain.other.SpecialFreight;
...
public class GoodsDao {
    ...
    private Boolean isOk(GoodsPo goodsPo){
        try {
            if (goodsPo.getGoodsCategoryId() != null && goodsCategoryMapper.getGoodsCategoryById(goodsPo.getGoodsCategoryId()) == null) {
                return false;
            }
            if (goodsPo.getBrandId() != null && brandMapper.getBrandById(goodsPo.getBrandId()) == null) {
                return false;
            }
            if(goodsPo.getBeSpecial()!=null&&goodsPo.getBeSpecial()){
                ...
                String success="0";
                String errno= JacksonUtil.parseString(body,"errno");
                if(!success.equals(errno)){
                    return false;
                }
                SpecialFreight specialFreight= JacksonUtil.parseObject(body,"data", SpecialFreight.class);
                return specialFreight != null;
            }
            return true;
        }catch (Exception e){
            return null;
    ...
    private List<GoodsPo> listBySomething(Integer id, Integer page, Integer limit, Integer type){
        ...
        if(hasKey!=null&&hasKey){
            ...
            Integer size= Math.toIntExact(boundZSetOperations.size());
            ...
            /*这是查询范围没有超过redis的存储范围的情况*/
                ...
                Set<String>goodsKeys=boundZSetOperations.range(start,start+limit);
                return redisTemplate.opsForValue().multiGet(goodsKeys);
            }
            else{
                Set<String>goodsKeys=boundZSetOperations.range(start,-1);
    ...
    private void removeSomething(String key,Integer type){
        BoundZSetOperations<String,String> boundZSetOperations= stringRedisTemplate.boundZSetOps(key);
        Set<String>keys=boundZSetOperations.range(0,-1);
