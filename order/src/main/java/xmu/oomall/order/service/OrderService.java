...
import com.google.gson.*;
import ooad33.order.controller.inter.*;
import xmu.oomall.order.controller.inter.*;
import xmu.oomall.order.dao.OrderDao;
import xmu.oomall.order.domain.Order;
import xmu.oomall.order.domain.OrderItem;
import ooad33.order.domain.otherdomin.*;
import xmu.oomall.order.domain.otherdomin.CartItem;
import xmu.oomall.order.domain.otherdomin.GrouponRulePo;
import xmu.oomall.order.domain.otherdomin.Payment;
import xmu.oomall.order.domain.otherdomin.PresaleRule;
...
public class OrderService {
    ...
    public Object submitOrder(Order order,List<CartItem> cartItems){
        ...
        if(this.createOrderItemByCartItem(order,cartItems)){
            ...
            Integer freight=JacksonUtil.parseInteger(JacksonUtil.toJson(frightController.getFright(order)),"data");
            ...
            Object object1=rocketMqProvider.addOrder(order);
    ...
    public Order orderCancel(Order order){
        ...
        String result=JacksonUtil.toJson(goodsController.updateDecute(true,order.getOrderItemList()));
        String field="data";
        if(JacksonUtil.parseBoolean(result,field)){
